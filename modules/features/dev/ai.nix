  ...
  flake.homeModules.ai =
    ...
    in
    {
      ...
      systemd.user.services.gemma = lib.mkIf (hostName == "aiServer") {
        ...
        Service = {
          ExecStart = "${llama}/bin/llama-server -hf ${consts.models.qwen35b} -c 32768 -ngl 99 --host 0.0.0.0 --port 8033 --spec-type draft-mtp --spec-draft-n-max 3 --webui-mcp-proxy --jinja -t 4 -tb 4 -np 1 --kv-unified --no-mmproj";
          Restart = "on-failure";
          RestartSec = "30s";
          StartLimitBurst = 3;
